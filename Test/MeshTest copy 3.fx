...
VS_OUTPUT sph_bump_spec_vs_main(VS_INPUT_MESH In) // Vertex
{
    ...
    //Out.ViewVector = normalize((float3)m_worldViewInv[2]);
...
float4 bump_spec_colorize_ps_main3(VS_OUTPUT In): COLOR // Pixel
{
    ...
    */
	...
	float nDotv = saturate(dot(N, In.ViewVector));


    // Cook-Torrance BRDF
    float3 F0 = lerp(float3(0.04, 0.04, 0.04), baseTexel.rgb, normalTexel.b);
    ...
    float3 kS = F;
    float3 kD = max((1.0 - kS) * (1.0 - normalTexel.b), 0.05); // Prevents pure black
    //float3 kD = (1.0 - kS) * max((1.0 - normalTexel.b), 0.1); // Prevents pure black

    float3 numerator = NDF * G * F;
    float  denominator = 4.0 * max(nDotv, 0.0) * max(nDotv, 0.0) + 0.001;

    float3 diffuse = kD * baseTexel.rgb * (nDotl * m_light0Diffuse.rgb * Diffuse.rgb + In.Diff.rgb) * 2.0;
    float3 specular = (numerator / denominator) * (nDotl * normalTexel.a * m_light0Specular * Specular * pow(nDoth,16));
    float3 reflection = nDotl * reflectTexel * F * normalTexel.b;
...
float4 bump_spec_colorize_ps_main(VS_OUTPUT In): COLOR // Pixel
{
    ...
    float3 diffuse = baseTexel.rgb * (NdotL * Diffuse.rgb * m_light0Diffuse.rgb * m_lightScale.rgb + In.Diff.rgb) * 2.0;
    float3 specular = (normalTexel.a * m_light0Specular * Specular * pow(NdotH,16));

    float3 reflection = reflectTexel * Metallic * NdotL;
...
float4 bump_spec_colorize_ps_main2(VS_OUTPUT In): COLOR
{
    ...
    diff = lerp(diff, In.Fog, m_fogVals.x);

	return float4(m_lightScale);
}
